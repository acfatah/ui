...
Every `size`. The `icon` sizes are square and expect a single icon with an `aria-label`.
...
Icons come from `@/components/ui/icons`, the single icon indirection, and take `data-part="icon"`.
...
`loading` marks the button busy and blocks activation without setting native `disabled`, so focus stays on the button. The spinner is slot content you supply.
...
`as-child` renders the single child element instead of a `<button>`, merging props and styles onto it. Use it for links that look like buttons.
