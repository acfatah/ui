...
## This site: `acfatah/ui` docs

Project rules. Where they disagree with the generic Nimbus guidance below,
these win.

- **Bun only.** Package manager, script runner and runtime. Run
  `bunx --bun nimbus-docs ...` from `apps/docs`, where the local bin
  resolves. The unscoped `nimbus-docs` on npm is a different, legacy
  package, so `bunx nimbus-docs` from anywhere else may fetch the wrong
  thing.
- **The sidebar is explicit** (`astro.config.ts`), not autogenerated. A new
  guide page must be added to the "Getting started" list. Component groups
  come from `categories` in each
...
- **Colours come from `shared/styles/global.css`.** `src/styles/globals.css`
  aliases Nimbus `--nb-*` tokens onto it; do not reintroduce colour values
  for names the shared layer defines. Dark mode sets both `data-mode` and
  `.dark` (BaseLayout).
- **Lint covers only files this app authors**; Nimbus-owned files are
  ignored in `eslint.config.ts` so upstream diffs stay clean.
...
| Goal | Action |
|---|---|
| New doc page | Create `src/content/docs/<slug>.mdx`, then add it to `sidebar.items` in `astro.config.ts`. |
| New partial | Create `src/content/partials/<slug>.mdx`. Use via `<Render file="<slug>" />`. |
| UI from registry | `bunx --bun nimbus-docs add <slug>`. Register in `src/components.ts` if used in MDX. |
| Feature recipe | `bunx --bun nimbus-docs add <feature-slug>`. Pipe the printed brief to your agent. |
| Check it builds | `bunx --bun nimbus-docs check` — build-free preflight (env + structure + authoring + types). `--json` for an agent loop, `--fix` to repair what's safe. |
| Custom page route | Add a file under `src/pages/`. |
| Custom OG style | Edit `src/pages/og/_og-card-config.ts`. |
| Check for updates | `bunx --bun nimbus-docs outdated` — starter files behind their tag + registry components behind. |
| Upgrade Nimbus | Update the package, then run `bunx --bun nimbus-docs migrate --dry-run --diff`. Review every change and required manual step before applying. |
| Upgrade a starter file | `bunx --bun nimbus-docs diff <file>` to review, `diff --apply <file>` to pull a clean upstream change. |
| Upgrade a registry component | `bunx --bun nimbus-docs add <slug> --overwrite`, then review with `git diff`. |
...
List installable items: `bunx --bun nimbus-docs list`.
...
2. Preview the complete plan with `bunx --bun nimbus-docs migrate --dry-run --diff`. If no baseline exists yet, add `--from <previous-version>`.
3. Review every versioned entry and resolve each blocked/manual item.
4. Apply safe edits only with explicit consent: `bunx --bun nimbus-docs migrate --yes`. Review the resulting diff, then rerun the preview.
5. When no migration remains, run `bunx --bun nimbus-docs migrate --yes` again to record the completed review in `nimbus.json`.
6. Run the project's typecheck and production build, then run `bunx --bun nimbus-docs check` again for post-build coverage.
...
Start with `bunx --bun nimbus-docs check --json`. It runs the environment, structural, authoring, and type checks build-free — config validity, `site` placeholder, route collisions, MDX component resolution, the lint rules, and a `tsc` type-check — and returns three top-level signals plus per-scope detail:
